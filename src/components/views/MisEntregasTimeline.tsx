...
import { ENTREGAS_MOCK } from "@/lib/mock-tareas";
import { cn } from "@/lib/utils";
import { colorCategoria, labelCategoria } from "@/lib/categorias";
...
const colorEstadoBar = (vencida: boolean, cerrada: boolean) => {
  if (cerrada) return "bg-green-500/50";
  if (vencida) return "bg-red-500";
  return "bg-blue-500";
};

export function MisEntregasTimeline() {
  useOverrides();
  ...
  const lista = useMemo(
    () =>
      [...ENTREGAS_MOCK]
        ...
        .sort(
          (a, b) =>
            parseISO(a.fecha_fin).getTime() - parseISO(b.fecha_fin).getTime(),
        ),
    [],
  );
    ...
    <TooltipProvider delayDuration={200}>
      <div className="card-soft overflow-hidden">
        <div className="overflow-x-auto">
          <div style={{ width: N_DIAS * DAY_W + 240 }}>
            {/* Header */}
            ...
            <div className="relative">
              ...
              {lista.map((e) => {
                ...
                return (
                  <div key={e.id} className="flex items-center border-b border-border/40" style={{ height: ROW_H }}>
                    ...
                    <div className="relative flex-1" style={{ height: ROW_H }}>
                      <Barra
                        entregaId={e.id}
                        titulo={e.nombre}
                        color={colorEstadoBar(vencida, false)}
                        left={left + 3}
                        width={width}
                        inicio={inicio}
                        fin={fin}
              ...
              {lista.length === 0 && (
                <EstadoVacio emoji="🌴" titulo="Sin entregas en curso" hint="Cuando haya entregas activas aparecerán aquí." />
              )}
            </div>
          </div>
        </div>
...
function Barra({
  entregaId, titulo, color, left, width, inicio, fin, onClick,
}: {
  entregaId: string; titulo: string; color: string; left: number; width: number;
  inicio: Date; fin: Date; onClick: () => void;
}) {
  const [drag, setDrag] = React.useState<null | { mode: "move" | "left" | "right"; startX: number; deltaDays: number; moved: boolean }>(null);
  ...
  const onPointerUp = (e: React.PointerEvent) => {
    ...
    if (drag.mode === "move") setEntregaFechas(entregaId, { inicio: shift(inicio, d), fin_max: shift(fin, d) });
    else if (drag.mode === "left") setEntregaFechas(entregaId, { inicio: shift(inicio, d) });
    else setEntregaFechas(entregaId, { fin_max: shift(fin, d) });
    toast.success("Fecha actualizada", { description: titulo });
    setDrag(null);
  };
  const offset = drag ? drag.deltaDays * DAY_W : 0;
  const visualLeft = drag?.mode === "left" ? left + offset : drag?.mode === "right" ? left : left + offset;
  const visualWidth = drag?.mode === "left" ? Math.max(DAY_W * 0.5, width - offset) : drag?.mode === "right" ? Math.max(DAY_W * 0.5, width + offset) : width;
  ...
  return (
    <Tooltip open={drag?.moved || undefined}>
      <TooltipTrigger asChild>
        ...
        >
          {titulo}
          <span onPointerDown={onPointerDown("left")} className="absolute left-0 top-0 h-full w-1.5 cursor-ew-resize opacity-0 group-hover:opacity-100 bg-black/30 rounded-l-sm" />
          <span onPointerDown={onPointerDown("right")} className="absolute right-0 top-0 h-full w-1.5 cursor-ew-resize opacity-0 group-hover:opacity-100 bg-black/30 rounded-r-sm" />
        </div>
      </TooltipTrigger>
      <TooltipContent>
        <div className="text-xs">
          <div className="font-semibold">{titulo}</div>
          <div>{format(previewInicio, "d MMM", { locale: es })} → {format(previewFin, "d MMM", { locale: es })}</div>
        </div>
      </TooltipContent>
    </Tooltip>
  );

...
import { useRevisionGlobal, useMisRevisiones } from "@/lib/queries";
...
function RevisionGlobalPage() {
  const { user } = useAuth();
  const caps = useUserCaps();
  if (!caps.isDirector) {
    return <Navigate to="/mis-revisiones" />;
  }
  return <RevisionContent userId={user?.id} caps={caps} />;
}
...
function RevisionContent({
  ...
  caps: ReturnType<typeof useUserCaps>;
}) {
  const [alcance, setAlcance] = useAlcancePersistido("revision", defaultAlcance(caps));
  const { data: globalRev = [] } = useRevisionGlobal();
  const { data: misRev = [] } = useMisRevisiones();
  const base = alcance === "mis-proyectos" ? misRev : globalRev;
  const tareas = React.useMemo(
    () => filtrarPorAlcance(base, alcance, userId, caps.clientesPM),
    [base, alcance, userId, caps.clientesPM],
  );
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="card-soft overflow-hidden">
        ...
        {tareas.length === 0 && (
          <EstadoVacio emoji="✅" titulo="Cola vacía" hint="No hay tareas pendientes de aprobación." />
        )}
      </div>
      {sel.size > 0 && (
